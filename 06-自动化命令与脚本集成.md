...
![difficulty](https://img.shields.io/badge/难度-⭐⭐_基础-green) ![time](https://img.shields.io/badge/阅读时间-19_分钟-blue) ![chapter](https://img.shields.io/badge/章节-06%2F21-purple)


> **难度**: ⭐⭐ 基础 | **预计阅读**: 20 分钟 | **前置章节**: [第 1-2 章](01-基础介绍与安装.md)
...
| 常见错误 | 表现 | 解决方法 |
|---------|------|----------|
| 脚本路径使用相对路径 | Cron 执行时找不到文件 | 始终使用绝对路径或 `$OPENCLAW_HOME` 变量 |
| 未处理命令失败 | 后续步骤在前置失败后仍执行 | 添加 `set -e` 或逐条检查 `$?` |
| 输出日志缺失 | 排查问题无据可查 | 使用 `openclaw run --log` 自动记录执行日志 |
| 并发执行冲突 | 多个脚本同时修改同一配置 | 使用文件锁机制或 `openclaw lock` 防止并发 |
...
| 执行方式 | 适用场景 | 特点 |
|---------|---------|------|
| 单次命令 `openclaw run` | 简单一次性任务 | 执行完即退出 |
| Cron 定时 | 周期性自动化 | 后台调度，支持失败重试 |
| Hook 触发 | 事件驱动自动化 | 文件变更、消息到达时触发 |
| Pipeline 编排 | 多步骤工作流 | 支持并行、条件分支、错误恢复 |
...
| 常见错误 | 后果 | 正确做法 |
|---------|------|----------|
| 硬编码路径 | 换环境后脚本失败 | 使用 `~/.openclaw` 或环境变量 |
| 未处理命令失败 | 后续步骤基于错误状态执行 | 使用 `set -e` 和错误检查 |
| 日志不记录 | 出问题时无法回溯 | 重定向输出到日志文件 |
...
| 执行方式 | 适用场景 | 特点 |
|---------|---------|------|
| openclaw run | 简单一次性任务 | 执行完即退出 |
| Cron 定时 | 周期性自动化 | 支持失败重试 |
| Hook 触发 | 事件驱动 | 文件变更时触发 |
| Pipeline 编排 | 多步骤工作流 | 支持并行和分支 |
